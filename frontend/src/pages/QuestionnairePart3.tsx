...
export default function QuestionnairePart3() {
  ...
  const bmiInfo = useMemo(() => {
    ...
    if (bmi < 18.5) {
      category = 'Underweight'; colorClass = 'text-blue-500 bg-blue-50 shadow-blue-500/20';
      gaugePercent = (bmi / 40) * 100;
    } else if (bmi < 25) {
      category = 'Normal'; colorClass = 'text-emerald-500 bg-emerald-50 shadow-emerald-500/20';
      gaugePercent = (bmi / 40) * 100;
    } else if (bmi < 30) {
      category = 'Overweight'; colorClass = 'text-amber-500 bg-amber-50 shadow-amber-500/20';
      gaugePercent = (bmi / 40) * 100;
    } else {
      category = 'Obese'; colorClass = 'text-rose-500 bg-rose-50 shadow-rose-500/20';
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-20">
        <div className="mx-auto flex h-16 max-w-3xl items-center px-4 sm:px-6">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-extrabold text-slate-800 tracking-tight">Medical History</h1>
          </div>
          <div className="text-xs font-bold tracking-widest text-teal-600 uppercase">Step 3 of 3</div>
        </div>
      ...
      <main className="flex-1 mx-auto w-full max-w-3xl px-4 py-8 sm:px-6">
        ...
        <div className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 p-8 sm:p-10 mb-8 animate-fade-in-up">
          <h2 className="text-xl font-black text-slate-900 mb-8">Vitals & BMI</h2>
          
          <div className="grid gap-8 md:grid-cols-2 items-center">
            <div className="space-y-6">
              <div className="space-y-1.5">
                <label className="block text-sm font-bold text-slate-700 ml-1">Height (cm)</label>
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-bold text-slate-700 ml-1">Weight (kg)</label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none transition-colors group-focus-within:text-teal-600 text-slate-400">
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-slate-50 rounded-3xl border border-slate-100 h-full relative overflow-hidden">
              {!bmiInfo ? (
                <div className="text-center text-slate-400">
                  ...
                  <p className="font-medium text-sm">Enter vitals to calculate</p>
                </div>
              ) : (
                <div className="text-center z-10 animate-fade-in">
                  <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-2">Calculated BMI</p>
                ...
                {bmiInfo && (
                  <div 
                    className={`h-full transition-all duration-1000 ease-out ${
                      bmiInfo.category === 'Underweight' ? 'bg-blue-500' :
                      bmiInfo.category === 'Normal' ? 'bg-emerald-500' :
                      bmiInfo.category === 'Overweight' ? 'bg-amber-500' : 'bg-rose-500'
                    }`}
        ...
        <div className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 p-8 sm:p-10 animate-fade-in-up-delay-1">
          <h2 className="text-xl font-black text-slate-900 mb-6">Additional Risk Factors</h2>
          ...
          <div className="space-y-4">
            <label className="flex items-center justify-between p-5 border-2 rounded-2xl border-slate-100 bg-slate-50 hover:bg-white hover:border-teal-200 transition-all cursor-pointer group has-[:checked]:border-teal-500 has-[:checked]:bg-teal-50/30">
              <span className="text-slate-800 font-bold group-hover:text-teal-700 transition-colors">Previous knee injury / trauma</span>
            ...
            <label className="flex items-center justify-between p-5 border-2 rounded-2xl border-slate-100 bg-slate-50 hover:bg-white hover:border-teal-200 transition-all cursor-pointer group has-[:checked]:border-teal-500 has-[:checked]:bg-teal-50/30">
              <span className="text-slate-800 font-bold group-hover:text-teal-700 transition-colors">Previous knee surgery</span>
              <div className="relative inline-flex items-center">
